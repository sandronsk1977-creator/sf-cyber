...
import { Network, ShieldHalf, ArrowRight, Hourglass } from "lucide-react";
import { Link } from "wouter";

export function PortalCardsSection() {
  return (
    <section id="portal" className="py-20 bg-slate-950 text-slate-100 relative hidden">
      <div className="container">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          ...
          {/* Card 02 - Cibersegurança */}
          <div className="relative p-8 rounded-3xl bg-slate-900/60 border border-slate-800 flex flex-col justify-between transition-all">
            <div className="flex items-start justify-between mb-6">
              <div className="w-14 h-14 rounded-2xl bg-blue-600/15 flex items-center justify-center text-blue-400">
            ...
            <h3 className="font-extrabold text-2xl mb-3 text-white font-mono">
              Cibersegurança · Trilhas e Cursos
            </h3>
            <p className="text-slate-400 text-sm leading-relaxed mb-8">
              Trilhas estruturadas do zero ao avançado, laboratórios práticos, simulados de exame e certificações verificáveis. Nossa área de cibersegurança está chegando.
            </p>

            <div className="flex items-center justify-between">
              <span className="inline-flex items-center gap-2 font-semibold bg-slate-800 text-slate-400 text-sm px-5 py-3 rounded-xl cursor-not-allowed opacity-60">
                <Hourglass className="w-4 h-4" />
                Em breve
              </span>
              <span className="text-xs text-slate-500 font-mono">Trilhas · Labs · Simulados</span>
            </div>
          </div>
        </div>
      </div>
